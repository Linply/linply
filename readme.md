...
- 认证：邮箱密码注册登录、数据库 Session、用户与管理员权限隔离
...
1. 设置生产环境变量，尤其是 `DATABASE_URL`、LLM 和 embedding 配置。
...
5. 使用 `NODE_ENV=production pnpm start` 启动服务并检查登录、用户隔离与管理员页面。
